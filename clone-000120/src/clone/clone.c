...
**
**	$Id: clone.c,v 1.82 2000/01/16 23:37:57 root Exp $
*/
...
global void SaveAll(void)
{
    ...
    fprintf(file,";;; -----------------------------------------\n");
    fprintf(file,";;; Save file generated by ALE Clone Version " VERSION "\n");
    time(&now);
...
global void CloneInit(void)
{
	...
	case TilesetSummer:
            PalettePath = strdcat(CloneLibPath, "/summer.rgb");
	    break;
	case TilesetWinter:
            PalettePath = strdcat(CloneLibPath, "/winter.rgb");
	    break;
	case TilesetWasteland:
            PalettePath = strdcat(CloneLibPath, "/wasteland.rgb");
	    break;
	case TilesetSwamp:
            PalettePath = strdcat(CloneLibPath, "/swamp.rgb");
	    break;
	default:
...
global char* CloneLibPath;		/// Path for data
...
map is relative to CloneLibPath=datapath, use ./map for relative\n\
",NameLine);
}
...
#endif
{
    ...
    //	Setup some defaults.
    //
    CloneLibPath=CLONE_LIB_PATH;
    TitleScreen=strdup("graphic/title.png");
    // CloneLibPath prefixed after it's been decided properly for the next two
    MapName="default.pud";	// .gz automatic appended, 
#ifdef USE_CCL
    ...
    //
    for( ;; ) {
	switch( getopt(argc,argv,"c:d:f:hlp:s:t:FW?") ) {
#ifdef USE_CCL
	    case 'c':
		CclStartFile=optarg;
		continue;
    ...
    if (MapName[0] != '/' && MapName[0] != '.') {
        MapName = strdcat3(CloneLibPath, "/", MapName);
    }
#ifdef USE_CCL
    if (CclStartFile[0] != '/' && CclStartFile[0] != '.') {
        CclStartFile = strdcat3(CloneLibPath, "/", CclStartFile);
    }
#endif
