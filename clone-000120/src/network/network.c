...
**
**	$Id: network.c,v 1.18 2000/01/03 21:41:46 root Exp $
*/
	...
	int position,unsigned x,unsigned y,const Unit* dest,const char* value)
{
	...
	fprintf(logf,";;; Log file generated by ALE Clone Version "
		VERSION "\n");
	time(&now);
    ...
    fprintf(logf,"(log %d 'U%Zd '%s '%s",
	    FrameCounter,unit-UnitsPool,name,
	    flag ? "flush" : "append");
    if( position ) {
	fprintf(logf," (%d %d)",x,y);
    }
    if( dest ) {
	fprintf(logf," 'U%Zd",unit-UnitsPool);
    }
    if( value ) {
