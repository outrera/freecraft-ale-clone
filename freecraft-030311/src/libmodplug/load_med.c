 ...
 *
 *	$Id: load_med.c,v 1.1 2001/11/07 23:36:13 johns Exp $
*/
...
typedef struct tagMMD0SONGHEADER
{
    ...
    int8_t playtransp;			// Play transpose
...
int CSoundFile_ReadMed(CSoundFile * that, const uint8_t * lpStream,
...
//---------------------------------------------------------------
{
    ...
    if (((pmmh->id & 0x00FFFFFF) != 0x444D4D) || (!pmmh->song)) {
	return 0;
    }
    // Check for 'MMDx'
    dwSong = bswapBE32(pmmh->song);
    if (dwSong >= dwMemLength
	|| (dwSong + sizeof(MMD0SONGHEADER) >= dwMemLength)) {
	return 0;
    }
    version = (int8_t) ((pmmh->id >> 24) & 0xFF);
	...
	Log("  playseq	  = ");
	for (unsigned idbg1 = 0; idbg1 < 16; idbg1++)
	    Log("%2d, ", pmsh->playseq[idbg1]);
	Log("...\n");
	Log("  deftempo	  = 0x%04X\n", bswapBE16(pmsh->deftempo));
	Log("  playtransp = %d\n", (int8_t) pmsh->playtransp);
	Log("  flags(1,2) = 0x%02X, 0x%02X\n", pmsh->flags, pmsh->flags2);
	Log("  tempo2	  = %d\n", pmsh->tempo2);
	Log("  trkvol	  = ");
	for (unsigned idbg2 = 0; idbg2 < 16; idbg2++)
	    Log("0x%02X, ", pmsh->trkvol[idbg2]);
	Log("...\n");
	...
	Log("  stereosep  = %d\n", (int8_t) pmsh2->mix_stereosep);
	Log("  deftempo	  = 0x%04X\n", bswapBE16(pmsh2->deftempo));
	Log("  playtransp = %d\n", (int8_t) pmsh2->playtransp);
    ...
    that->m_nChannels = 4;
    that->m_nSamples = pmsh->numsamples;
    if (that->m_nSamples > 63) {
	that->m_nSamples = 63;
    }
    // Tempo
	...
	deftempo = 125;
    if (pmsh->flags2 & MMD_FLAG2_BPM) {
	unsigned tempo_tpl = (pmsh->flags2 & MMD_FLAG2_BMASK) + 1;
	...
	Log("newtempo: %3d bpm (bpm=%3d lpb=%2d)\n", deftempo,
	    bswapBE16(pmsh->deftempo), (pmsh->flags2 & MMD_FLAG2_BMASK) + 1);
#endif
    } else {
    ...
    // Speed
    that->m_nDefaultSpeed = pmsh->tempo2;
    if (!that->m_nDefaultSpeed)
	that->m_nDefaultSpeed = 6;
    if (deftempo < 0x21)
	deftempo = 0x21;
    if (deftempo > 255) {
	while ((that->m_nDefaultSpeed > 3) && (deftempo > 260)) {
	    deftempo =
		(deftempo * (that->m_nDefaultSpeed -
		    1)) / that->m_nDefaultSpeed;
	    that->m_nDefaultSpeed--;
	}
	if (deftempo > 255)
	    deftempo = 255;
    }
	...
	pins->nLoopStart = bswapBE16(pmsh->sample[i].rep) << 1;
	pins->nLoopEnd =
	    pins->nLoopStart + (bswapBE16(pmsh->sample[i].replen) << 1);
	pins->nVolume = (pmsh->sample[i].svol << 2);
	pins->nGlobalVol = 64;
	if (pins->nVolume > 256) {
	    pins->nVolume = 256;
	}
	pins->RelativeTone = -12 * pmsh->sample[i].strans;
	pins->nPan = 128;
	if (pins->nLoopEnd) {
    ...
    if (version < '2') {
	unsigned nbo = pmsh->songlen >> 8;

	if (nbo >= MAX_ORDERS)
	    nbo = MAX_ORDERS - 1;
	if (!nbo)
	    nbo = 1;
	memcpy(that->Order, pmsh->playseq, nbo);
	...
	unsigned nTrks;
	unsigned iSection;
	uint32_t playseqtable;
	unsigned numplayseqs;
	uint32_t sectiontable;

	nTrks = bswapBE16(pmsh2->numtracks);
	if ((nTrks >= 4) && (nTrks <= 32)) {
	    that->m_nChannels = nTrks;
	}
	...
	if (!nSections || !sectiontable || (sectiontable >= dwMemLength - 2)) {
	    nSections = 1;
	}
	nOrders = 0;
	for (iSection = 0; iSection < nSections; iSection++) {
		...
		pseq =
		    bswapBE32(((uint32_t *) (lpStream +
			    playseqtable))[nplayseq]);
	    }
	    if ((pseq) && (pseq < dwMemLength - sizeof(MMD2PLAYSEQ))) {
		MMD2PLAYSEQ *pmps = (MMD2PLAYSEQ *) (lpStream + pseq);
		unsigned n;

		if (!that->m_szNames[0][0]) {
		    memcpy(that->m_szNames[0], pmps->name, 31);
		}
		n = bswapBE16(pmps->length);

		if (pseq + n <= dwMemLength) {
		    for (i = 0; i < n; i++) {
			unsigned seqval = pmps->seq[i] >> 8;

			if ((seqval < wNumBlocks)
			    && (nOrders < MAX_ORDERS - 1)) {
			    that->Order[nOrders++] = seqval;
			}
		    }
		}
	    }
	...
	unsigned annolen;
	unsigned songname;
	unsigned songnamelen;
	uint32_t smpinfoex;
	uint32_t trackinfo_ofs;

	// Channel Split
	if ((that->m_nChannels == 4) && (pmsh->flags & 0x40)) {
	    for (i = 0; i < 4; i++) {
		if (pmex->channelsplit[i]) {
		    that->m_nChannels++;
		}
	    }
	}
	// Song Comments
	annotxt = bswapBE32(pmex->annotxt);
	annolen = bswapBE32(pmex->annolen);

	if ((annotxt) && (annolen) && (annotxt + annolen <= dwMemLength)) {
	    that->m_lpszSongComments = malloc(annolen + 1);

	    memcpy(that->m_lpszSongComments, lpStream + annotxt, annolen);
	    that->m_lpszSongComments[annolen] = 0;
	}
	// Song Name
	songname = bswapBE32(pmex->songname);
	songnamelen = bswapBE32(pmex->songnamelen);

	if ((songname) && (songnamelen)
	    && (songname + songnamelen <= dwMemLength)) {
	    if (songnamelen > 31)
		songnamelen = 31;
	    memcpy(that->m_szNames[0], lpStream + songname, songnamelen);
	}
	// Sample Names
	smpinfoex = bswapBE32(pmex->iinfo);

	if (smpinfoex) {
	    uint32_t iinfoptr = bswapBE32(pmex->iinfo);
	    unsigned ientries = bswapBE16(pmex->i_ext_entries);
	    unsigned ientrysz = bswapBE16(pmex->i_ext_entrsz);

	    if ((iinfoptr) && (ientrysz < 256)
		&& (iinfoptr + ientries * ientrysz < dwMemLength)) {
		const char *psznames = (const char *)(lpStream + iinfoptr);
		unsigned maxnamelen = ientrysz;

		if (maxnamelen > 32)
		    maxnamelen = 32;
		for (i = 0; i < ientries; i++)
		    if (i < that->m_nSamples) {
			strncpy(that->m_szNames[i + 1],
			    psznames + i * ientrysz, maxnamelen);
		    }
	    }
	}
	// Track Names
	trackinfo_ofs = bswapBE32(pmex->trackinfo_ofs);

	if ((trackinfo_ofs)
	    && (trackinfo_ofs + that->m_nChannels * 4 < dwMemLength)) {
		...
		if (trktagofs) {
			...
			ntag = bswapBE32(*(uint32_t *) (lpStream + trktagofs));
			if (ntag == MMDTAG_END)
			    break;
			tagdata =
			    bswapBE32(*(uint32_t *) (lpStream + trktagofs + 4));
			switch (ntag) {
		    ...
		    if ((trknameofs)
			&& (trknameofs + trknamelen < dwMemLength)) {
			strncpy(that->ChnSettings[i].szName,
			    (const char *)(lpStream + trknameofs),
			    MAX_CHANNELNAME);
		    }
		}
	    }
    ...
    // Reading samples
    if (dwSmplArr > dwMemLength - 4 * that->m_nSamples) {
	return 1;
    }
    pdwTable = (uint32_t *) (lpStream + dwSmplArr);
    for (i = 0; i < that->m_nSamples; i++) {
	if (pdwTable[i]) {
	    unsigned dwPos = bswapBE32(pdwTable[i]);
	    unsigned flags;
	    unsigned stype;
	    char *psdata;
	    ...
	    if ((dwPos >= dwMemLength)
		|| (dwPos + sizeof(MMDSAMPLEHEADER) >= dwMemLength)) {
		continue;
	    }
		...
		bswapBE16(psdh->type), len);
#endif
	    if ((len > MAX_SAMPLE_LENGTH) || (dwPos + len + 6 > dwMemLength)) {
		len = 0;
	    }
	    flags = RS_PCM8S;
	    stype = bswapBE16(psdh->type);
	    psdata = (char *)(lpStream + dwPos + 6);

	    if (stype & 0x80) {
		psdata += (stype & 0x20) ? 14 : 6;
	    } else {
		if (stype & 0x10) {
		    that->Ins[i + 1].uFlags |= CHN_16BIT;
		    len /= 2;
	    ...
	    that->Ins[i + 1].nLength = len;
	    CSoundFile_ReadSample(that,&that->Ins[i + 1], flags, psdata,
		dwMemLength - dwPos - 6);
	}
    }
    // Reading patterns (blocks)
    if (wNumBlocks > MAX_PATTERNS) {
	wNumBlocks = MAX_PATTERNS;
    }
    if ((!dwBlockArr) || (dwBlockArr > dwMemLength - 4 * wNumBlocks)) {
	return 1;
    }
	...
	if (!dwPos || dwPos >= dwMemLength || dwPos >= dwMemLength - 8) {
	    continue;
	}
	//lines = 64;
	//tracks = 4;
	...
	if (version == '0') {
	    ...
	    if (!tracks) {
		tracks = that->m_nChannels;
	    }
	    if ((that->Patterns[i] =
		    CSoundFile_AllocatePattern(lines,
			that->m_nChannels)) == NULL) {
		continue;
	    }
	    that->PatternSize[i] = lines;
	    p = that->Patterns[i];
	    s = (uint8_t *) (lpStream + dwPos + 2);
	    maxlen = tracks * lines * 3;

	    if (maxlen + dwPos > dwMemLength - 2)
		break;
	    for (y = 0; y < lines; y++) {
		unsigned x;

		for (x = 0; x < tracks; x++, s += 3)
		    if (x < that->m_nChannels) {
			...
			if (s[0] & 0x40)
			    instr |= 0x20;
			if ((note) && (note <= 132))
			    p->note = note + playtransp;
			p->instr = instr;
			p->command = s[1] & 0x0F;
			p->param = s[2];
			// if (!i) Log("%02X.%02X.%02X | ", s[0], s[1], s[2]);
			MedConvert(p, pmsh);
			p++;
		    }
		//if (!i) Log("\n");
		...
		bswapBE16(pmb->lines), bswapBE16(pmb->numtracks),
		bswapBE32(pmb->info));
#endif
	    lines = (pmb->lines >> 8) + 1;
	    ...
	    if ((that->Patterns[i] =
			...
			that->m_nChannels)) == NULL)
		continue;
	    that->PatternSize[i] = (uint16_t) lines;
		...
		if ((pbi->blockname) && (pbi->blocknamelen)) {
		    uint32_t nameofs = bswapBE32(pbi->blockname);
		    unsigned namelen = bswapBE32(pbi->blocknamelen);

		    if ((nameofs < dwMemLength)
			&& (nameofs + namelen < dwMemLength)) {
			CSoundFile_SetPatternName(that,i,
			    (const char *)(lpStream + nameofs));
		    }
		}
		if (pbi->cmdexttable) {
			...
			cmdexttable =
			    bswapBE32(*(uint32_t *) (lpStream + cmdexttable));
			if ((cmdexttable)
			    && (cmdexttable <= dwMemLength - lines * tracks)) {
	    ...
	    p = that->Patterns[i];
	    s = (uint8_t *) (lpStream + dwPos + 8);
	    maxlen = tracks * lines * 4;

	    if (maxlen + dwPos > dwMemLength - 8) {
		break;
	    }
	    for (y = 0; y < lines; y++) {
		unsigned x;

		for (x = 0; x < tracks; x++, s += 4) {
		    if (x < that->m_nChannels) {
			uint8_t note = s[0];
