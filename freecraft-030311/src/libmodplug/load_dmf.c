 ...
 *
 *	$Id: load_dmf.c,v 1.3 2002/04/17 23:16:01 johns Exp $
*/
...
{
    uint8_t tracks;
    uint8_t beat;			// [hi|lo] -> hi=ticks per beat
					//	      lo=beats per measure
...
int CSoundFile_ReadDMF(CSoundFile * that, const uint8_t * lpStream,
...
//---------------------------------------------------------------
{
    ...
    if ((pfh->id != 0x464d4444) || (!pfh->version) || (pfh->version & 0xF0)) {
	return 0;
    }
    dwMemPos = 66;
    memcpy(that->m_szNames[0], pfh->songname, 30);
    that->m_szNames[0][30] = 0;
    that->m_nType = MOD_TYPE_DMF;
    that->m_nChannels = 0;
#ifdef DMFLOG
    Log("DMF version %d: \"%s\": %d bytes (0x%04X)\n", pfh->version,
	that->m_szNames[0], dwMemLength, dwMemLength);
#endif
	...
	switch (id) {
		...
		if ((psi->infosize > dwMemLength)
		    || (psi->infosize + dwMemPos + 8 > dwMemLength)) {
		    goto dmfexit;
		}
		if ((psi->infosize >= 8) && (!that->m_lpszSongComments)) {
		    that->m_lpszSongComments = malloc(psi->infosize);
		    if (that->m_lpszSongComments) {
			for (i = 0; i < psi->infosize - 1; i++) {
			    int8_t c = lpStream[dwMemPos + 8 + i];

			    if ((i % 40) == 39) {
				that->m_lpszSongComments[i] = 0x0d;
			    } else {
				that->m_lpszSongComments[i] =
				    (c < ' ') ? ' ' : c;
			    }
			}
			that->m_lpszSongComments[psi->infosize - 1] = 0;
		    }
		}
		...
		if ((sequ->seqsize >= dwMemLength)
		    || (dwMemPos + sequ->seqsize + 12 > dwMemLength))
		    goto dmfexit;
		{
		    unsigned nseq = sequ->seqsize >> 1;

		    if (nseq >= MAX_ORDERS - 1)
			nseq = MAX_ORDERS - 1;
		    if (sequ->loopstart < nseq)
			that->m_nRestartPos = sequ->loopstart;
		    for (i = 0; i < nseq; i++) {
			that->Order[i] = (uint8_t) sequ->sequ[i];
		    }
		}
		...
		if (!that->m_nChannels) {
		    ...
		    if ((patt->patsize >= dwMemLength)
			|| (dwMemPos + patt->patsize + 8 > dwMemLength))
			goto dmfexit;
		    numpat = patt->numpat;
		    if (numpat > MAX_PATTERNS)
			numpat = MAX_PATTERNS;
		    that->m_nChannels = patt->tracks;
		    if (that->m_nChannels < patt->firstpatinfo)
			that->m_nChannels = patt->firstpatinfo;
		    if (that->m_nChannels > 32)
			that->m_nChannels = 32;
		    if (that->m_nChannels < 4)
			that->m_nChannels = 4;
		    for (npat = 0; npat < numpat; npat++) {
			DMFTRACK *pt;
			unsigned ticks;
			unsigned tracks;
			MODCOMMAND *m;
			...
			unsigned ttype;
			unsigned tempo;
			unsigned glbinfobyte;
			unsigned pbeat;
			int tempochange;
			unsigned row;

			pt = (DMFTRACK *) (lpStream + dwPos);
#ifdef DMFLOG
			...
			dwPos += 8;
			if ((pt->jmpsize >= dwMemLength)
			    || (dwPos + pt->jmpsize + 4 >= dwMemLength)) {
			    break;
			}
			that->PatternSize[npat] = (uint16_t) ticks;

			m = CSoundFile_AllocatePattern(that->PatternSize[npat],
			    that->m_nChannels);
			if (!m) {
			    goto dmfexit;
			}
			...
			tempochange = (pt->beat & 0xf0) ? 1 : 0;

			memset(infobyte, 0, sizeof(infobyte));
			for (row = 0; row < ticks; row++) {
			    MODCOMMAND *p = &m[row * that->m_nChannels];
				...
				if ((info & 0x80) && (d < dwPos))
				    glbinfobyte = lpStream[d++];
				info &= 0x7f;
				if ((info) && (d < dwPos))
				    infoval = lpStream[d++];
				switch (info) {
			    ...
			    if (tempochange) {
				unsigned speed = 6, modtempo = tempo;
				unsigned rpm = ((ttype)
				    && (pbeat)) ? tempo * pbeat : (tempo +
				    1) * 15;
				...
				for (i = 0; i < that->m_nChannels; i++)
					...
					if (speed) {
					    ...
					    p[i].param = (uint8_t) speed;
					    speed = 0;
					} else if ((modtempo >= 32)
					    && (modtempo < 256)) {
					    p[i].command = CMD_TEMPO;
					    p[i].param = (uint8_t) modtempo;
					    modtempo = 0;
					} else {
					    break;
					}
		...
		if (pds->size <= dwMemLength - dwMemPos) {
			...
			if (dwPos + namelen + 1 + sizeof(DMFSAMPLE) >
			    dwMemPos + pds->size + 8) {
			    break;
			}
			if (namelen) {
			    unsigned rlen = (namelen < 32) ? namelen : 31;

			    memcpy(that->m_szNames[i], lpStream + dwPos + 1,
				rlen);
			    that->m_szNames[i][rlen] = 0;
			}
			dwPos += namelen + 1;
			...
			psmp->nGlobalVol = 64;
			psmp->nVolume =
			    (psh->volume) ? ((uint16_t) psh->volume) +
			    1 : (uint16_t) 256;
			...
			Log("SMPI %d/%d: len=%d flags=0x%02X\n", i,
			    that->m_nSamples, psmp->nLength, psh->flags);
#endif
		    }
		}
		dwMemPos += pds->size + 8;
		...
		for (i = 1; i <= that->m_nSamples; i++) {
			...
			Log("Unexpected EOF at sample %d/%d! (pos=%d)\n", i,
			    that->m_nSamples, dwPos);
#endif
			break;
		    }
		    pksize = *((uint32_t *) (lpStream + dwPos));
#ifdef DMFLOG
		    Log("sample %d: pos=0x%X pksize=%d ", i, dwPos, pksize);
		    Log("len=%d flags=0x%X [%08X]\n", that->Ins[i].nLength,
			smplflags[ismpd],
			*((uint32_t *) (lpStream + dwPos + 4)));
#endif
		    dwPos += 4;
		    ...
		    if ((pksize) && (i <= that->m_nSamples)) {
			unsigned flags =
			    (that->Ins[i].
			    uFlags & CHN_16BIT) ? RS_PCM16S : RS_PCM8S;

			if (smplflags[ismpd] & 4)
			    flags =
				(that->Ins[i].
				uFlags & CHN_16BIT) ? RS_DMF16 : RS_DMF8;
			CSoundFile_ReadSample(that,&that->Ins[i], flags,
			    (char *)(lpStream + dwPos), pksize);
		    }
		    dwPos += pksize;
		}
		dwMemPos = dwPos;
...
static uint8_t DMFReadBits(DMF_HTREE * tree, unsigned nbits)
//-------------------------------------------
{
    uint8_t x = 0, bitv = 1;
...
int CSoundFile_DMFUnpack(uint8_t * psample, uint8_t * ibuf, uint8_t * ibufmax,
    unsigned maxlen)
//----------------------------------------------------------------------
{
    DMF_HTREE tree;
