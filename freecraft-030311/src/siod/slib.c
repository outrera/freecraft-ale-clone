...
#ifndef __GNUC__
#define __attribute__(args)    /* args */
#endif

static void init_slib_version(void)
{setvar(cintern("*slib-version*"),
	cintern("$Id: slib.c,v 1.21 2003/02/21 12:58:39 cedars Exp $"),
	NIL);}
...
LISP freelist;
jmp_buf errjmp;
long errjmp_ok = 0;    /* temporary disabled */
long errjmp_avail = 0; /* errjmp buf was set with setjmp */
long nointerrupt = 1;
...
#endif
#ifdef USE_BEOS
#define SIOD_LIB_DEFAULT "/boot/home/config/lib/siod"
#endif
#endif
...
#ifndef _WIN32_WCE
void handle_sigfpe(int sig SIG_restargs __attribute__((unused)))
{
#ifdef WIN32
...
#include <float.h>
 ...
 errl("floating point exception",NIL);}
#endif

#ifndef _WIN32_WCE
void handle_sigint(int sig SIG_restargs __attribute__((unused)))
...
{int k;
 ...
 LISP* old_stack_start_ptr;
 char* old_stack_limit_ptr;

 old_stack_start_ptr = stack_start_ptr;
 old_stack_limit_ptr = stack_limit_ptr;
...
static void ignore_print(LISP x __attribute__((unused)))
 ...
 *p->ptr = 0;
 if (cplen != fromlen)
   errl("repl_c_string_print overflow",NIL);
 return(1);}
...
LISP errswitch(void)
{return(errl("BUG. Reached impossible case",NIL));}

void err_stack(char *ptr __attribute__((unused)))
     /* The user could be given an option to continue here */
{errl("the currently assigned stack limit has been exceded",NIL);}
...
{if (CONSP(message) && TYPEP(CAR(message),tc_string))
   ...
   errl(get_c_string(message),x);
 return(NIL);}

void gc_fatal_error(void)
{errl("ran out of storage",NIL);}
...
{switch TYPE(x)
   {case tc_nil:
    ...
    case tc_cons:
      return(CDR(x));
    default:
      return(errl("wta to cdr",x));}}

LISP setcar(LISP cell, LISP value)
{if NCONSP(cell) errl("wta to setcar",cell);
 return(CAR(cell) = value);}

LISP setcdr(LISP cell, LISP value)
{if NCONSP(cell) errl("wta to setcdr",cell);
...
{if NULLP(y)
 ...
 if NFLONUMP(x) errl("wta(1st) to plus",x);
 if NFLONUMP(y) errl("wta(2nd) to plus",y);
 return(flocons(FLONM(x) + FLONM(y)));}

LISP ltimes(LISP x,LISP y)
{if NULLP(y)
   return(NULLP(x) ? flocons(1) : x);
 if NFLONUMP(x) errl("wta(1st) to times",x);
 if NFLONUMP(y) errl("wta(2nd) to times",y);
 return(flocons(FLONM(x)*FLONM(y)));}

LISP difference(LISP x,LISP y)
{if NFLONUMP(x) errl("wta(1st) to difference",x);
 if NULLP(y)
   return(flocons(-FLONM(x)));
 else
   {if NFLONUMP(y) errl("wta(2nd) to difference",y);
    return(flocons(FLONM(x) - FLONM(y)));}}

LISP Quotient(LISP x,LISP y)
{if NFLONUMP(x) errl("wta(1st) to quotient",x);
 if NULLP(y)
   return(flocons(1/FLONM(x)));
 else
   {if NFLONUMP(y) errl("wta(2nd) to quotient",y);
    return(flocons(FLONM(x)/FLONM(y)));}}

LISP lllabs(LISP x)
{double v;
 if NFLONUMP(x) errl("wta to abs",x);
 v = FLONM(x);
 if (v < 0)
   return(flocons(-v));
 else
...
LISP lsqrt(LISP x)
{if NFLONUMP(x) errl("wta to sqrt",x);
 return(flocons(sqrt(FLONM(x))));}

LISP greaterp(LISP x,LISP y)
{if NFLONUMP(x) errl("wta(1st) to greaterp",x);
 if NFLONUMP(y) errl("wta(2nd) to greaterp",y);
 if (FLONM(x)>FLONM(y)) return(sym_t);
 return(NIL);}

LISP lessp(LISP x,LISP y)
{if NFLONUMP(x) errl("wta(1st) to lessp",x);
 if NFLONUMP(y) errl("wta(2nd) to lessp",y);
 if (FLONM(x)<FLONM(y)) return(sym_t);
 return(NIL);}
...
{if NULLP(y) return(x);
 if NFLONUMP(x) errl("wta(1st) to max",x);
 if NFLONUMP(y) errl("wta(2nd) to max",y);
 return((FLONM(x) > FLONM(y)) ? x : y);}

LISP lmin(LISP x,LISP y)
{if NULLP(y) return(x);
 if NFLONUMP(x) errl("wta(1st) to min",x);
 if NFLONUMP(y) errl("wta(2nd) to min",y);
 return((FLONM(x) < FLONM(y)) ? x : y);}
...
LISP err_ubv(LISP v)
{return(errl("unbound variable",v));}

LISP symbol_boundp(LISP x,LISP env)
{LISP tmp;
 if NSYMBOLP(x) errl("not a symbol",x);
 tmp = envlookup(x,env);
...
{LISP l,tmp;
 ...
 return(errl("improper list to assq",alist));}
...
{long n;
 ...
 if ((type >= 0) && (type < tc_table_dim))
   ...
   errl("type number out of range",NIL);
 return(NULL);}

long allocate_user_tc(void)
{long x = user_tc_next;
 if (x > tc_user_max)
   errl("ran out of user type codes",NIL);
 ++user_tc_next;
 return(x);}
...
{if (gc_kind_copying == 1)
   errl("cannot perform operation with stop-and-copy GC mode. Use -g0\n",
       NIL);}
...
{LISP result,v1,v2,tmp;
 ...
 if NCONSP(l) errl("bad syntax argument list",l);
 ...
 for(v1=result,v2=CDR(l);
  ...
  {tmp = cons(leval(CAR(v2),env),NIL);
   CDR(v1) = tmp;}
 if NNULLP(v2) errl("bad syntax argument list",l);
 return(result);}
...
{LISP frame,al,fl,tmp;
 for(frame=env;CONSP(frame);frame=CDR(frame))
   {tmp = CAR(frame);
    if NCONSP(tmp) errl("damaged frame",tmp);
    for(fl=CAR(tmp),al=CDR(tmp);CONSP(fl);fl=CDR(fl),al=CDR(al))
      {if NCONSP(al) errl("too few arguments",tmp);
 ...
 if NNULLP(frame) errl("damaged env",env);
 return(NIL);}
...
{return(errl("closure code type not valid",tmp));}
...
{struct user_type_hooks *p;
 ...
 switch TYPE(fcn)
   {case tc_subr_0:
    ...
    case tc_msubr:
    case tc_symbol:
      return(errl("cannot be applied",fcn));
    case tc_closure:
    ...
    default:
      ...
      if (p->leval)
	return(errl("have eval, dont know apply",fcn));
      else
	return(errl("cannot be applied",fcn));}}

LISP setvar(LISP var,LISP val,LISP env)
{LISP tmp;
 if NSYMBOLP(var) errl("wta(non-symbol) to setvar",var);
...
{LISP tmp,var,val;
 ...
 var = car(tmp);
 if NSYMBOLP(var) errl("wta(non-symbol) to define",var);
...
{struct catch_frame *l;
 for(l=catch_framep; l; l = (*l).next)
   if (EQ((*l).tag,tag) ||
     ...
     {(*l).retval = value;
      longjmp((*l).cframe,2);}
 errl("no *catch found with this tag",tag);
 return(NIL);}
...
LISP leval_quote(LISP args,LISP env __attribute__((unused)))
{return(car(args));}

LISP leval_tenv(LISP args __attribute__((unused)),LISP env)
{return(env);}
...
{long size;
 ...
 for(l=args;NNULLP(l);l=cdr(l))
   {s = car(l);
    if NSYMBOLP(s) errl("wta(non-symbol) to symbolconc",s);
    size = size + strlen(PNAME(s));
    if (size >  TKBUFFERN) errl("symbolconc buffer overflow",NIL);
...
void z_ungetc(int c, CLFile *f __attribute__((unused)))
{zz_char_valid=1;
  zz_char=c;}


LISP lreadz(CLFile *f)
{struct gen_readio s;
 s.getc_fcn = (int (*)(void *)) z_getc;
 s.ungetc_fcn = (void (*)(int,void *)) z_ungetc;
 s.cb_argument = (char *) f;
 return(readtl(&s));}
#endif
...
{int c,j;
 ...
 switch (c)
   {case '(':
      return(lreadparen(f));
    case ')':
      errl("unexpected close paren",NIL);
    case '\'':
 ...
 *p++ = c;
 for(j = 1; j<TKBUFFERN; ++j)
    ...
    *p++ = c;}
 return(errl("token larger than TKBUFFERN",NIL));}

#ifndef SIOD_RECURSION
...
{LISP total, *pos, tmp;
 ...
 if EQ(tmp,sym_dot) 
  {tmp = lreadr(f);
   c = flush_ws(f,"end of file inside list");
   if (c != ')') errl("missing close paren",NIL);
   return(tmp);}
 pos = &total;
 do 
  {NEWCELL(*pos, tc_cons);
...
{long flag;
 flag = no_interrupt(1);
 if NTYPEP(p,tc_c_file) errl("not a file",p);
 file_gc_free(p);
 no_interrupt(flag);
 return(NIL);}
...
{char *cname,*chow = NULL;
 ...
 else errl("bad argument to save-forms",how);
 if (siod_verbose_level >= 3)
...
LISP arglchk(LISP x)
{
 ...
 if NNULLP(l) errl("improper formal argument list",x);
#endif
 return(x);}
...
{if (NULLP(p) && deflt) return(deflt);
 if NTYPEP(p,tc_c_file) errl("not a file",p);
 if (!p->storage_as.c_file.f) errl("file is closed",p);
...
static LISP parser_read(LISP ignore __attribute__((unused)))
