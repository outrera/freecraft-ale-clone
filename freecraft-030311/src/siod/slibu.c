...
{setvar(cintern("*slibu-version*"),
	cintern("$Id: slibu.c,v 1.18 2002/12/10 08:08:53 nehalmistry Exp $"),
	NIL);}
...
#if defined(unix) || defined(WIN32) || defined(__APPLE__) || defined(USE_BEOS)
...
{char path[PATH_MAX+1];
 if (getcwd(path,sizeof(path)))
   ...
   return(errl("getcwd",llast_c_errmsg(-1)));}
...
{uid_t uid;
 ...
 if (setuid(uid))
   return(errl("setuid",llast_c_errmsg(-1)));
 else
   return(NIL);}
...
{uid_t uid;
 uid = (uid_t) get_c_long(n);
 if (seteuid(uid))
   return(errl("seteuid",llast_c_errmsg(-1)));
 else
   return(NIL);}
...
{int iflag = no_interrupt(1);
 ...
 putpwent(&p,get_c_file(file,NULL));
#endif /* !__FreeBSD__ */
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if (symlink(get_c_string(p1),get_c_string(p2)))
   return(errl("symlink",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if (link(get_c_string(p1),get_c_string(p2)))
   return(errl("link",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if (unlink(get_c_string(p)))
   return(errl("unlink",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if (rmdir(get_c_string(p)))
   return(errl("rmdir",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if (mkdir(get_c_string(p),get_c_long(m)))
   return(errl("mkdir",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag;
 ...
 if ((size = readlink(get_c_string(p),buff,sizeof(buff))) < 0)
   return(errl("readlink",llast_c_errmsg(-1)));
 no_interrupt(iflag);
...
{long iflag;
 ...
 if (rename(get_c_string(p1),get_c_string(p2)))
   return(errl("rename",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
LISP lrandom(LISP n)
{int res;
#if defined(hpux) || defined(vms) || defined(sun) || defined(sgi) || defined(WIN32) || defined(__dietlibc__)
 res = rand();
#else
#if defined(__osf__) || defined(BSD) || defined(linux) || defined(USE_BEOS)
 res = random();
#endif
#endif
...
{int iflag;
 ...
 if (pid == 0)
   {no_interrupt(iflag);
    return(NIL);}
 if (pid == -1)
   return(errl("fork",llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(flocons(pid));}
...
{int iflag;
 ...
 no_interrupt(iflag);
 return(errl("exec",llast_c_errmsg(-1)));}
...
{int result = 0,val,noptions,nmask = 0;
 ...
 if ((noptions == -1) ||
   ...
   errl("contains undefined options",l);
 return(result);}
...
{pid_t pid,ret;
 ...
 if (ret == 0)
   ...
   return(errl("wait",llast_c_errmsg(-1)));
 else
...
LISP ltrunc(LISP x)
{long i;
 if NFLONUMP(x) errl("wta to trunc",x);
 i = (long) FLONM(x);
...
{char *orig,*cpy;
 ...
 strcpy(cpy,orig);
  if (putenv(cpy))
   return(errl("putenv",llast_c_errmsg(-1)));
 else
   return(NIL);}
#endif
...
{char *buffer; long dim,n;
 ...
 if TYPEP(len,tc_c_file)
   ...
   {n = get_c_long(len);
    if ((n < 0) || (n > dim)) errl("invalid length for string",len);}
...
{struct rlimit x;
 if (getrlimit(RLIMIT_CPU,&x))
   {errjmp_ok = 0;
    errl("getrlimit",llast_c_errmsg(-1));}
 if (x.rlim_cur >= x.rlim_max)
   {errjmp_ok = 0;
    errl("hard cpu limit exceded",NIL);}
 if (nointerrupt == 1)
   interrupt_differed = 1;
 else
   errl("cpu limit exceded",NIL);}

LISP cpu_usage_limits(LISP soft,LISP hard)
{struct rlimit x;
 if (NULLP(soft) && NULLP(hard))
   {if (getrlimit(RLIMIT_CPU,&x))
      return(errl("getrlimit",llast_c_errmsg(-1)));
    else
   ...
   {x.rlim_cur = get_c_long(soft);
    ...
    if (setrlimit(RLIMIT_CPU,&x))
      return(errl("setrlimit",llast_c_errmsg(-1)));
    else
      return(NIL);}}
...
void handle_sigalrm(int sig __attribute__((unused)))
{if (nointerrupt == 1)
...
{struct rusage u;
 ...
 if (NULLP(kind) || EQ(cintern("SELF"),kind))
   ...
   return(errl("unknown rusage",kind));
 if (getrusage(code,&u))
   return(errl("getrusage",llast_c_errmsg(-1)));
...
#if defined(unix) || defined(__APPLE__) || defined(USE_BEOS)
...
{long iflag;
 ...
 if (!(d = opendir(get_c_string(name))))
   return(errl("opendir",llast_c_errmsg(-1)));
...
{if NTYPEP(v,tc_opendir) errl("not an opendir",v);
 if NULLP(CAR(v))
   {if (oflag) errl("opendir not open",v);
    return(NULL);}
 return((DIR *)CAR(v));}
...
{long iflag,old_errno;
 ...
 CAR(v) = NIL;
 if (closedir(d))
   return(errl("closedir",llast_c_errmsg(old_errno)));
 no_interrupt(iflag);
 return(NIL);}
...
{long iflag,namlen;
 ...
 no_interrupt(iflag);
 if (!r) return(NIL);
#if defined(sun) || defined(sgi) || defined(linux) || defined(USE_BEOS)
...
{long iflag;
 ...
 if ((d->h = FindFirstFile(get_c_string(name),&d->s)) == INVALID_HANDLE_VALUE)
   return(errl("FindFirstFile",llast_win32_errmsg(0)));
 no_interrupt(iflag);
 return(value);}
...
{long iflag;
 ...
 if ((h != INVALID_HANDLE_VALUE) && !FindClose(h))
   return(errl("closedir",llast_win32_errmsg(0)));
 no_interrupt(iflag);
 return(NIL);}
...
#if defined(unix) || defined(WIN32) || defined(__APPLE__)
...
LISP siod_chmod(LISP path,LISP mode)
{if (chmod(get_c_string(path),get_c_long(mode)))
   return(errl("chmod",llast_c_errmsg(-1)));
 else
   return(NIL);}
...
{struct utimbuf x;
 ...
 if (utime(get_c_string(fname), &x))
   return(errl("utime",llast_c_errmsg(-1)));
 else
   return(NIL);}
...
{if (fchmod(fileno(get_c_file(file,NULL)),get_c_long(mode)))
   return(errl("fchmod",llast_c_errmsg(-1)));
 else
   return(NIL);}
...
// FIXME: Some solaris have usleep
...
{FILE *f;
 ...
 if NULLP(buffn)
   ...
   errl("size must be >= 0",buffn);
 else if ((unsigned)n > sizeof(buffer))
   errl("not handling buffer of size",listn(2,buffn,flocons(sizeof(buffer))));
 iflag = no_interrupt(1);
...
LISP siod_chown(LISP path,LISP uid,LISP gid)
{long iflag;
 iflag = no_interrupt(1);
 if (chown(get_c_string(path),get_c_long(uid),get_c_long(gid)))
   errl("chown",cons(path,llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
...
LISP siod_lchown(LISP path,LISP uid,LISP gid)
{long iflag;
 iflag = no_interrupt(1);
 if (lchown(get_c_string(path),get_c_long(uid),get_c_long(gid)))
   errl("lchown",cons(path,llast_c_errmsg(-1)));
 no_interrupt(iflag);
 return(NIL);}
#endif
...
{FILE *f = get_c_file(ptr,NULL);
 ...
 no_interrupt(iflag);
 if (retval < 0)
   errl("pclose",llast_c_errmsg(xerrno));
 return(flocons(retval));}
...
{LISP init_name;
 ...
 if (!(handle = dlopen(get_c_string(fname),RTLD_LAZY)))
   errl(dlerror(),fname);
 if (!(fcn = (void (*)(void))dlsym(handle,get_c_string(init_name))))
   errl(dlerror(),init_name);
#endif
#if defined(hpux)
 if (access(get_c_string(fname),F_OK))
   fname = string_append(listn(3,
 ...
 if (!(handle = shl_load(get_c_string(fname),BIND_DEFERRED,0L)))
   errl("shl_load",llast_c_errmsg(errno));
 if (shl_findsym(&handle,get_c_string(init_name),TYPE_PROCEDURE,&fcn))
   errl("shl_findsym",llast_c_errmsg(errno));
#endif
#if defined(VMS)
 ...
 if (!(handle = LoadLibrary(get_c_string(fname))))
   errl("LoadLibrary",fname);
 if (!(fcn = (LPVOID)GetProcAddress(handle,get_c_string(init_name))))
   errl("GetProcAddress",init_name);
#endif
 if (fcn)
   (*fcn)();
 else
   errl("did not load function",init_name);
 no_interrupt(iflag);
 if (siod_verbose_check(3))
   put_st("done.\n");
 return(init_name);}
...
{time_t b;
 ...
 if ((t = localtime(&b)))
   ...
   return(errl("localtime",llast_c_errmsg(-1)));}
#endif
...
{time_t b;
 ...
 if ((t = gmtime(&b)))
   ...
   return(errl("gmtime",llast_c_errmsg(-1)));}
#endif
...
{long iflag;
 ...
 switch(TYPE(dir))
   {case tc_c_file:
      ...
      iflag = no_interrupt(1);
      if (fchdir(fd))
	return(errl("fchdir",llast_c_errmsg(-1)));
      no_interrupt(iflag);
#else
      errl("fchdir not supported in os",NIL);
#endif
      return(NIL);
    default:
      ...
      iflag = no_interrupt(1);
      if (chdir(path))
	return(errl("chdir",llast_c_errmsg(-1)));
      no_interrupt(iflag);
      return(NIL);}}
...
{int filedes[2];
 ...
 if (pipe(filedes) == 0)
   ...
   return(errl("pipe",llast_c_errmsg(-1)));}
#endif
...
{return(errl("index too large",ind));}
...
{char *data;
 ...
 i = get_c_long(ind);
 if (i < 0) errl("negative index",ind);
 switch(get_c_long(ctype))
   {case CTYPE_FLOAT:
    ...
    default:
      return(errl("unknown CTYPE",ctype));}}
...
#endif
#if defined(unix) || defined(__APPLE__)
 // JOHNS: init_subr_2("crypt",lcrypt);
